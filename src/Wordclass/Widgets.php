...
class Widgets {
     ...
     */
    public function addSidebar($id, $name, $description=null, $widget=[], $title=[]) {
        add_action('widgets_init', function() use($id, $name, $description, $widget, $title) {
            $description = $description ?: $name;

            register_sidebar([
                'id'            => $id,
                'name'          => __($name, static::$_textDomainStatic),
                'description'   => __('Sidebar: ' . $description, static::$_textDomainStatic),
     ...
     * Instead of settings options, a name (string) can be given, using default options
     */
